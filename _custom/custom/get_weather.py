...
GET_WEATHER_FUNCTION_DESC = {
    ...
    "function": {
        ...
        "description": (
            ...
            "If no location given, defaults to Bangkok."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "location": {
                    "type": "string",
                    "description": "City or place name in any language, e.g. กรุงเทพ / Bangkok / เชียงใหม่",
                },
                "lang": {
                    "type": "string",
                    "description": "Language code of the user, e.g. th_TH / en_US / zh_CN. Default th_TH",
                },
            },
            "required": ["lang"],
        },
...
def _weatherapi_report(api_key, location, lang_short):
    ...
    if r.status_code == 400 and r.json().get("error", {}).get("code") == 1006:
        return None  # location not found — let caller report it
    r.raise_for_status()
    data = r.json()
...
# ---------- OpenWeatherMap (fallback tier 2) ----------

def _owm_report(api_key, location, lang_short):
    base = "https://api.openweathermap.org/data/2.5"
    params = {"q": location, "appid": api_key, "units": "metric", "lang": lang_short}
    r = requests.get(f"{base}/weather", params=params, headers=HEADERS, timeout=10)
    if r.status_code == 404:
        return None  # location not found — let caller try next tier
    r.raise_for_status()
    cur = r.json()
    rf = requests.get(f"{base}/forecast", params=params, headers=HEADERS, timeout=10)
    rf.raise_for_status()
    fc = rf.json()
...
# ---------- Open-Meteo (fallback tier 3) ----------

def _geocode(location, lang_short):
...
def _openmeteo_report(location, lang_short):
    geo = _geocode(location, lang_short)
    if not geo:
        geo = _geocode(location, "en")
    if not geo:
        return None
...
def get_weather(conn: "ConnectionHandler", location: str = None, lang: str = "th_TH"):
    ...
    cache_key = f"wapi_weather_{location}_{lang_short}"
    ...
    if owm_key:
        try:
            report = _owm_report(owm_key, location, lang_short)
    ...
    if report is None and api_key:
        try:
            report = _weatherapi_report(api_key, location, lang_short)
        except (requests.RequestException, KeyError, ValueError) as e:
            logger.bind(tag=TAG).warning(
    ...
    if report is None:
        try:
            report = _openmeteo_report(location, lang_short)
