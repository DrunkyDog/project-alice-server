...
class ManageApiClient:
    ...
    def __new__(cls, config):
        """Singleton pattern ensures a globally unique instance and supports passing config parameters"""
    ...
    def _init_client(cls, config):
        ...
        if not cls.config.get("url") or not cls.config.get("secret"):
            raise Exception("Invalid manager-api url or secret configuration")
        ...
        # Do not create AsyncClient here; defer to actual usage
        cls._async_clients = {}

    @classmethod
    async def _ensure_async_client(cls):
        """Ensure the async client is created (create independent clients for each event loop)"""
        import asyncio
        ...
        try:
            ...
            # Create independent clients for each event loop
            if loop_id not in cls._async_clients:
                # The server may actively close connections; httpx connection pool cannot correctly detect and clean them
                limits = httpx.Limits(
    ...
    async def _async_request(cls, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        try:
            ...
            # Ensure the response is closed (executes even on exception)
        ...
        while retry_count <= cls.max_retries:
            try:
                ...
                if retry_count < cls.max_retries and cls._should_retry(e):
                    retry_count += 1
                    print(
                        f"{method} {endpoint} async request failed, retry {retry_count} in {cls.retry_delay:.1f} seconds"
                    )
...
async def lookup_address_book(caller_mac: str, nickname: str) -> Optional[Dict]:
    """Find target device by nickname"""
