...
class GlobalGCManager:
    ...
    async def start(self):
        ...
        logger.bind(tag=TAG).info(f"Started global GC manager, interval {self.interval_seconds} seconds")
    ...
    async def stop(self):
        ...
        logger.bind(tag=TAG).info("Stopped global GC manager")
    ...
    async def _run_gc(self):
        ...
        try:
            # Perform GC in thread pool to avoid blocking event loop
            ...
            logger.bind(tag=TAG).error(f"Error during GC execution: {e}")
