...
class GlobalCacheManager:
    ...
    def set(
        ...
        with self._locks[cache_name]:
            ...
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                # LRU strategy: move to end if already exists
                if key in cache:
                    del cache[key]
                cache[key] = entry
                ...
                # Check size limit
                if config.max_size and len(cache) > config.max_size:
                    # Simple strategy: remove one entry arbitrarily
    ...
    def _maybe_cleanup(self, cache_name: str):
        ...
        if now - self._last_cleanup > config.cleanup_interval:
            ...
            if deleted > 0:
                ...
                self.logger.debug(f"Cleaning cache {cache_name}: deleted {deleted} expired entries")
