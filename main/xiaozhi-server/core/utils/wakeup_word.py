...
class WakeupWordsConfig:
    ...
    def _load_config(self) -> Dict:
        """Load config file using cache mechanism"""
        ...
        try:
            ...
            print(f"Unknown error occurred while loading config file: {e}")
            return {}

    def _save_config(self, config: Dict):
        """Save config to file, protected by file lock"""
        try:
            ...
            print(f"Unknown error occurred while saving config file: {e}")
            raise
    ...
    def update_wakeup_response(self, voice: str, file_path: str, text: str):
        ...
        try:
            ...
            print(f"Failed to update wakeup word response config: {e}")
            raise

    def generate_file_path(self, voice: str) -> str:
        """Generate audio file path using hash of voice as filename"""
        try:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    print(f"Failed to remove existing audio file: {e}")
                    raise
