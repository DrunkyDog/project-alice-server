...
def initialize_modules(
    ...
    if init_intent:
        ...
        logger.bind(tag=TAG).info(f"Initialize component: intent success {select_intent_module}")
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(f"Initialize component: memory success {select_memory_module}")
...
def initialize_voiceprint(asr_instance, config):
    ...
    try:
        ...
        logger.bind(tag=TAG).info("ASR module voiceprint recognition dynamically enabled")
        logger.bind(tag=TAG).info(f"Configured speakers count: {len(voiceprint_config['speakers'])}")
        return True
    except Exception as e:
        logger.bind(tag=TAG).error(f"Dynamic initialization of voiceprint recognition failed: {str(e)}")
        return False
