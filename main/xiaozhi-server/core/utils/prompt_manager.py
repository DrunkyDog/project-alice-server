...
Responsible for managing and updating system prompts, including fast initialization and async enhancement
"""
...
class PromptManager:
    ...
    def _load_base_template(self):
        ...
        try:
            ...
            if os.path.exists(template_path):
                ...
                # Save to cache (CONFIG type default does not automatically expire, requires manual invalidation)
    ...
    def get_quick_prompt(self, user_prompt: str, device_id: str = None) -> str:
        ...
        if cached_device_prompt is not None:
            ...
            self.logger.bind(tag=TAG).debug(
                f"Device {device_id} has no cached prompt, using passed prompt"
            )

        # Use passed prompt and cache it (if device ID exists)
        if device_id:
            device_cache_key = f"device_prompt:{device_id}"
    ...
    def _get_weather_info(self, conn: "ConnectionHandler", location: str) -> str:
        ...
        try:
            ...
            # Therefore use call_soon_threadsafe to submit task + threading.Event to wait for result
            # Note: Event.wait() only blocks current thread pool thread, not main event loop
            ...
            if not event.wait(timeout=10):
                raise TimeoutError("Timeout fetching weather info")
            if exception_holder:
    ...
    def update_context_info(self, conn, client_ip: str):
        ...
        try:
            ...
            if (
                ...
                # Get location info (using global cache)
            ...
            if (
                ...
                # Get weather info (using global cache)
    ...
    def build_enhanced_prompt(
        ...
        try:
            # Get latest time info (no caching)
            ...
            # Get language selected for TTS, default value is Chinese
            language = (
                self.config.get("TTS", {})
            ...
            self.logger.bind(tag=TAG).debug(f"Retrieved selected language: {language}")
