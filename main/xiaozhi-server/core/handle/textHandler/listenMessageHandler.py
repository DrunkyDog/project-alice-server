    ...
    async def handle(self, conn: "ConnectionHandler", msg_json: Dict[str, Any]) -> None:
        ...
        if msg_json["state"] == "start":
            ...
            if conn.asr.interface_type == InterfaceType.STREAM:
                ...
                # In non-streaming mode: trigger ASR recognition directly
            ...
            if "text" in msg_json:
                ...
                # Check if it's a device call command [device_call]
                if original_text.startswith("[device_call]"):
                    ...
                    # Add to conversation history for model context
                    conn.dialogue.put(Message(role="assistant", content=call_text))
                    return

                # Check if it's a wakeup word
                is_wakeup_words = filtered_text in conn.config.get("wakeup_words")
                # Whether wakeup word reply is enabled
                enable_greeting = conn.config.get("enable_greeting", True)

                if is_wakeup_words and not enable_greeting:
                    # If it's a wakeup word and wakeup reply is disabled, don't respond
                    await send_stt_message(conn, original_text)
                    await send_tts_message(conn, "stop", None)
                    conn.client_is_speaking = False
                elif is_wakeup_words:
                    conn.just_woken_up = True
                    # Report text-only data (reuses ASR reporting, but without audio data)
                    enqueue_asr_report(conn, "Hi there!", [])
                    await startToChat(conn, "Hi there!")
                else:
                    conn.just_woken_up = True
                    # Report text-only data (reuses ASR reporting, but without audio data)
