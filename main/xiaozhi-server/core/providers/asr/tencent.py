...
class ASRProvider(ASRProviderBase):
    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.secret_id or not self.secret_key:
                logger.bind(tag=TAG).error("Tencent Cloud speech recognition is not configured; recognition cannot proceed")
                return None, None
            ...
            if result:
                logger.bind(tag=TAG).debug(
                    f"Tencent Cloud speech recognition took {time.time() - start_time:.3f}s | result: {result}"
                )

            return result, artifacts.file_path

        except Exception as e:
            logger.bind(tag=TAG).error(f"An error occurred while processing the audio! {e}", exc_info=True)
            return None, None
    ...
    def _build_request_body(self, base64_audio: str) -> str:
        ...
        request_map = {
            ...
            "EngSerViceType": "16k_zh",  # Mandarin general-purpose
            "SourceType": 1,  # Audio source is a speech file
    ...
    def _get_auth_headers(self, request_body: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Note: headers must be sorted in ASCII order, and both key and value must be lowercased.
            # The content-type and host headers must be included.
            content_type = "application/json; charset=utf-8"
            host = "asr.tencentcloudapi.com"
            action = "SentenceRecognition"  # Interface name

            # Build the canonical headers, keeping the order and format in mind.
            canonical_headers = (
...
logger.bind(tag=TAG).error(f"Failed to generate authentication headers: {e}", exc_info=True)
                raise RuntimeError(f"Failed to generate authentication headers: {e}")
    ...
    def _send_request(
        ...
        try:
            ...
            if "Response" in response_json and "Error" in response_json["Response"]:
                ...
                raise IOError(f"API returned an error: {error_code}: {error_message}")
            ...
            if "Response" in response_json and "Result" in response_json["Response"]:
                ...
                logger.bind(tag=TAG).warning(f"No recognition result was found in the response: {response_json}")
                return ""

        except Exception as e:
            logger.bind(tag=TAG).error(f"Request failed: {e}", exc_info=True)
            return None
