...
class CaptureOutput:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        # Output the captured content via the logger.
        if self.output:
...
class ASRProvider(ASRProviderBase):
    def __init__(self, config: dict, delete_audio_file: bool):
        ...
        # Memory check; at least 2 GB is required.
        min_mem_bytes = 2 * 1024 * 1024 * 1024
        try:
            total_mem = psutil.virtual_memory().total
        except RuntimeError as e:
            logger.bind(tag=TAG).warning(f"Failed to retrieve system memory information; skipping FunASR memory check: {e}")
        else:
            if total_mem < min_mem_bytes:
                logger.bind(tag=TAG).error(f"Available memory is below 2 GB; only {total_mem / (1024*1024):.2f} MB is available, so FunASR may not start")
