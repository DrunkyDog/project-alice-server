...
class ASRProviderBase(ABC):
    ...
    async def receive_audio(self, conn: "ConnectionHandler", pcm_frame, audio_have_voice):
        if conn.client_listen_mode == "manual":
            ...
            # If there is no speech and there was no previous speech, cache a portion of the audio.
            if not audio_have_voice and not conn.client_have_voice:
                conn.asr_audio = conn.asr_audio[-10:]
                return

            # In automatic mode, trigger recognition when VAD detects that speech has stopped.
            if conn.asr.interface_type != InterfaceType.STREAM and conn.client_voice_stop:
                # Use the PCM data directly from asr_audio.
                pcm_bytes = b"".join(conn.asr_audio)
                # Check whether there is enough audio data (1920 bytes per frame, about 28,800 bytes for 15 frames).
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            # The data is already PCM; use it directly.
            ...
            # Record the recognition result and check whether it is an exception.
            ...
            # Determine the ASR result type.
            if isinstance(raw_text, dict):
                # The dict format returned by FunASR.
                if speaker_name:
            ...
            if text_len > 0:
                ...
                # Send the report using the custom module.
    ...
    def _build_enhanced_text(self, text: str, speaker_name: Optional[str]) -> str:
        """Build text containing speaker information (used for plain-text ASR only)."""
    ...
    class AudioArtifacts(NamedTuple):
        ...
        """PCM audio frame list."""
        pcm_bytes: bytes
        """Merged PCM audio byte data."""
    ...
    def prefers_temp_file(self) -> bool:
        """Whether to prefer temporary files."""
        return False
    ...
    def build_temp_file(self, pcm_bytes: bytes) -> Optional[str]:
        try:
            ...
            logger.bind(tag=TAG).error(f"Temporary audio file generation failed: {e}")
            return None
    ...
    async def speech_to_text(
        ...
        :param artifacts: Audio artifacts, including PCM data, file paths, and so on
