...
class ASRProvider(ASRProviderBase):
    ...
    def _load_model(self):
        ...
        try:
            if not os.path.exists(self.model_path):
                raise FileNotFoundError(f"The VOSK model path does not exist: {self.model_path}")
                
            logger.bind(tag=TAG).info(f"Loading the VOSK model: {self.model_path}")
            self.model = vosk.Model(self.model_path)

            # Initialize the VOSK recognizer (the sample rate must be 16 kHz).
            self.recognizer = vosk.KaldiRecognizer(self.model, 16000)

            logger.bind(tag=TAG).info("VOSK model loaded successfully")
        except Exception as e:
            logger.bind(tag=TAG).error(f"Failed to load the VOSK model: {e}")
            raise
    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.model:
                logger.bind(tag=TAG).error("The VOSK model is not loaded; recognition cannot proceed")
                return "", None
            
            if artifacts is None:
                return "", None
            if not artifacts.pcm_bytes:
                logger.bind(tag=TAG).warning("The combined PCM data is empty")
                return "", None

            start_time = time.time()
            
            
            # Perform recognition (VOSK recommends sending 2000 bytes at a time).
            chunk_size = 2000
            text_result = ""
