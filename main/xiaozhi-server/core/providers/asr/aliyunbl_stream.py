...
class ASRProvider(ASRProviderBase):
    ...
    async def receive_audio(self, conn, pcm_frame, audio_have_voice):
        ...
        # Only establish a connection when there is audio and no connection exists.
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            # If manual mode is used, set the timeout to its maximum value.
            ...
            logger.bind(tag=TAG).debug(f"Connecting to the Aliyun Bailian ASR service, task_id: {self.task_id}")
            ...
            logger.bind(tag=TAG).debug("Sent the run-task command; waiting for the server to be ready...")
    ...
    def _build_run_task_message(self) -> dict:
        ...
        # Add the vocabulary_id parameter only when the model name ends with v2.
            ...
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    # Handle the result-generated event.
                        ...
                        if is_final:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # In manual mode, processing is triggered only after the stop signal is received.
                    ...
                    # Handle the task-finished event.
                        ...
                        logger.bind(tag=TAG).debug("The task is complete")
                        break
                    ...
                    # Handle the task-failed event.
                        ...
                        logger.bind(tag=TAG).error(f"The task failed: {error_code} - {error_message}")
                        break
                    ...
                    logger.bind(tag=TAG).error(f"Failed to process the results: {str(e)}")
                    break
    ...
    async def _send_stop_request(self):
        """Send a stop request (used to stop recording in manual mode)."""
        if self.asr_ws:
            try:
                # Stop sending audio first.
                self.is_processing = False

                logger.bind(tag=TAG).debug("Stop request received; sending the finish-task command")
    ...
    async def _send_finish_task(self):
        ...
        if self.asr_ws and self.task_id:
            try:
                ...
                logger.bind(tag=TAG).debug("The finish-task command has been sent")
            except Exception as e:
                logger.bind(tag=TAG).error(f"Failed to send the finish-task command: {e}")
    ...
    async def _cleanup(self):
        ...
        if self.asr_ws:
            try:
                # Send the finish-task command first.
                await self._send_finish_task()
