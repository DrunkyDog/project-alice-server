...
class OTAHandler(BaseHandler):
    ...
    async def handle_post(self, request):
        ...
        response = web.Response(
            text=json.dumps({"success": False, "message": "Internal error"}),
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # if configured with a non-empty string
                # Attempt to get device model from request data (already parsed above)
                try:
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # leave empty if signature fails, device decides whether to allow no password
                else:
                    self.logger.bind(tag=TAG).warning("Missing MQTT signature key, leaving password empty")

                # Build MQTT config (directly using mqtt_gateway string)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"Delivering MQTT gateway config to device {device_id}")

            else:  # mqtt_gateway not configured, deliver WebSocket
                # If authentication is enabled, perform authentication validation
                token = ""
                if self.auth_enable:
                ...
                self.logger.bind(tag=TAG).info(
                    f"MQTT gateway not configured, delivering WebSocket config to device {device_id}"
                )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Searching firmware for model {device_model}, found {len(candidates)} candidate(s)"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Delivering firmware {chosen_version} to device {device_id} [if address prefix is wrong, check server.vision_explain in config]-> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
            ...
            self.logger.bind(tag=TAG).error(f"OTA POST handling exception: {e}")
    ...
    async def handle_get(self, request):
        ...
        response = web.Response(text="Internal error", content_type="text/plain", status=500)
        try:
            server_config = self.config["server"]
            local_ip = get_local_ip()
            ...
            message = f"OTA endpoint running normally, websocket address sent to device: {websocket_url}"
            response = web.Response(text=message, content_type="text/plain")
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"OTA GET request exception: {e}")
            response = web.Response(text="OTA endpoint exception", content_type="text/plain")
        finally:
            self._add_cors_headers(response)
            return response
    ...
    async def handle_download(self, request):
        ...
        - Only allows downloading .bin files under data/bin directory
        - filename must be a basename matching a safe pattern
        """
        resp = web.Response(text="Internal error", status=500)
        try:
