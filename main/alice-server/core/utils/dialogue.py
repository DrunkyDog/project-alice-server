...
class Message:
    def __init__(
        ...
        self.is_temporary = is_temporary  # Flag temporary message (e.g. tool call reminder)
...
class Dialogue:
    ...
    def get_llm_dialogue(self) -> List[Dict[str, str]]:
        ...
        # This ensures speaker functionality works across all invocation paths
    ...
    def _ensure_tool_calls_complete(self, messages: List[Message]) -> List[Message]:
        """
        Ensure all tool_calls have corresponding tool responses
        Fix dangling tool_calls caused by interruption to prevent LLM API 400 errors
        """
    ...
    def get_llm_dialogue_with_memory(
        ...
        if system_message:
            ...
            try:
                ...
                # Only output speakers_info when valid identity is injected in this round, avoiding repeated names in list every round
                # which induces the model to address repeatedly; subsequent rounds do not inject identity, relying on first round in dialogue history
        ...
        # Second section: few-shot examples (unchanged in session)
