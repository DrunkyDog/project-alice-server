...
class VoiceprintProvider:
    ...
    def __init__(self, config: dict):
        ...
        if not self.original_url:
            ...
            if not self.api_key:
                ...
                # Construct identify endpoint address
    ...
    def _check_server_health(self) -> bool:
        ...
        # Use global cache manager to cache result
    ...
    async def identify_speaker(self, audio_data: bytes, session_id: str) -> Optional[str]:
        ...
        if not self.enabled or not self.api_url or not self.api_key:
            logger.bind(tag=TAG).debug("Voiceprint recognition disabled or unconfigured, skipping identification")
            return None
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.bind(tag=TAG).info(f"Voiceprint identification time taken: {total_elapsed_time:.3f}s")
                        ...
                        if speaker_id and speaker_id in self.speaker_map:
                            ...
                            logger.bind(tag=TAG).info(f"Voiceprint identification successful: {result_name} (Similarity: {score:.3f})")
                            return result_name
                        else:
            ...
            logger.bind(tag=TAG).error(f"Voiceprint identification timeout: {elapsed:.3f}s")
            return None
        except Exception as e:
            elapsed = time.monotonic() - api_start_time
