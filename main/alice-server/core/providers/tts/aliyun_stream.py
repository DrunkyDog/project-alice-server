...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        # If the configured address is an internal network (contains -internal.aliyuncs.com), use ws protocol, otherwise default to wss protocol
        if "-internal." in self.host:
            self.ws_url = f"ws://{self.host}/ws/v1"
        else:
            # Default to use wss protocol
    ...
    async def _ensure_connection(self):
        ...
        try:
            if self._is_token_expired():
                logger.bind(tag=TAG).warning("Token has expired, auto-refreshing...")
                self._refresh_token()
            current_time = time.time()
            if self.ws and current_time - self.last_active_time < 10:
                # Connection can only be reused within 10 seconds for continuous conversation
                self.task_id = uuid.uuid4().hex
