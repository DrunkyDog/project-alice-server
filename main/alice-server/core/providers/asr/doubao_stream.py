...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        self.is_processing = False  # Added processing-state flag
        self._is_stopping = False  # Added stop flag to prevent race conditions

        # Configuration parameters
        self.appid = str(config.get("appid"))
        self.access_token = config.get("access_token")
        # Resource ID used to distinguish different ASR models (default is the 1.0 model hourly version; v2 uses seed-asr)
    ...
    async def receive_audio(self, conn: "ConnectionHandler", pcm_frame, audio_have_voice):
        ...
        if audio_have_voice and self.asr_ws is None and not self.is_processing:
            try:
                ...
                logger.bind(tag=TAG).info(f"Connecting to the ASR service, headers: {headers}")
                ...
                try:
                    ...
                    logger.bind(tag=TAG).info(f"Sending the initialization request: {request_params}")
                    ...
                    logger.bind(tag=TAG).error(f"Failed to send the initialization request: {str(e)}")
                    if hasattr(e, "__cause__") and e.__cause__:
                        logger.bind(tag=TAG).error(f"Cause: {str(e.__cause__)}")
                    raise e

                # Start the asynchronous task to receive ASR results.
            ...
            while self.asr_ws and not conn.stop_event.is_set():
                ...
                try:
                    ...
                    if "payload_msg" in result:
                        ...
                        # Check whether it is error code 1013 (no valid speech).
                        if "code" in payload and payload["code"] == 1013:
                            # Silently ignore it and do not log an error.
                            continue
                        ...
                        if "result" in payload:
                            ...
                            if (
                                not self.enable_multilingual  # Note: multilingual mode does not return intermediate results; wait for the final result.
                            ...
                            # Handle recognition results with no text (in manual mode, recognition may already be complete but the button has not been released).
                            elif not payload["result"].get("text") and not utterances:
                                # Multilingual mode keeps returning empty text until the final full result is returned, so exclude it.
                                if self.enable_multilingual:
                                    continue

                                if conn.client_listen_mode == "manual" and conn.client_voice_stop and len(audio_data) > 15:
                                    logger.bind(tag=TAG).debug("Received a stop signal at the end of the message; triggering processing")
                                    await self.handle_voice_stop(conn, audio_data)
                                    break
                            ...
                            for utterance in utterances:
                                if utterance.get("definite", False):
                                    ...
                                    if conn.client_listen_mode == "manual":
                                        ...
                                        # If a stop signal is received mid-message.
                                        if conn.client_voice_stop and len(audio_data) > 0:
                                            logger.bind(tag=TAG).debug("Received a stop signal mid-message; triggering processing")
                                            await self.handle_voice_stop(conn, audio_data)
                                        break
                                    else:
                            ...
                            logger.bind(tag=TAG).error(f"The ASR service returned an error: {error_msg}")
                            break
    ...
    async def _send_stop_request(self):
        """Send the final audio frame to notify the server that the input has ended."""
        self._is_stopping = True  # Mark as stopping first to block subsequent audio sending.
        if self.asr_ws:
            try:
                ...
                logger.bind(tag=TAG).debug("Sent the end-of-stream audio frame")
            except Exception as e:
                logger.bind(tag=TAG).debug(f"An error occurred while sending the end-of-stream audio frame: {e}")
    ...
    def parse_response(self, res: bytes) -> dict:
        try:
            ...
            if len(res) < 4:
                logger.bind(tag=TAG).error(f"The response data length is insufficient: {len(res)}")
                return {"error": "The response data length is insufficient"}
            ...
            # Read the JSON data.
            try:
                # Check whether bytes 8-11 contain a valid JSON length field.
                ...
                if length > 0 and length <= len(res) - 12:
                    # There is a length field; read the specified length of JSON data starting at byte 12.
                    json_data = res[12:12 + length].decode("utf-8")
                else:
                    # There is no length field or it is invalid; try parsing directly.
            ...
            logger.bind(tag=TAG).error(f"Failed to parse the response: {str(e)}")
