...
class SuppressInvalidHandshakeFilter(logging.Filter):
    """Filter out invalid handshake error logs (for example, HTTPS access to a WS port)."""
...
def _setup_websockets_logger():
    """Configure all websockets-related loggers and filter out invalid handshake errors."""
...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        if headers.get("device-id", None) is None:
            ...
            if not request_path:
                self.logger.bind(tag=TAG).error("Could not obtain the request path")
                await websocket.close()
                return
            parsed_url = urlparse(request_path)
            query_params = parse_qs(parsed_url.query)
            if "device-id" not in query_params:
                await websocket.send("The port is working; to test the connection, start the digital-human test")
                await websocket.close()
                return
            else:
        ...
        """Handle a new connection by creating a separate ConnectionHandler each time."""
        ...
        try:
            ...
            await websocket.close()
            return
        # Pass the current server instance when creating the ConnectionHandler
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Error while handling the connection: {e}")
        finally:
            ...
            try:
                ...
                if hasattr(websocket, "closed") and not websocket.closed:
                    ...
                    # If there is no closed attribute, try closing directly
                    await websocket.close()
            except Exception as close_error:
                self.logger.bind(tag=TAG).error(
                    f"Error while force-closing the connection on the server side: {close_error}"
                )

    async def _http_response(self, websocket, request_headers):
        # Check whether this is a WebSocket upgrade request
        if request_headers.headers.get("connection", "").lower() == "upgrade":
            # If this is a WebSocket request, return None to allow the handshake to continue
            return None
        else:
            # If this is a regular HTTP request, return "server is running"
            return websocket.respond(200, "Server is running\n")

    async def update_config(self) -> bool:
        """Update the server configuration and reinitialize the components.
        ...
        try:
            async with self.config_lock:
                # Refresh the configuration (using the asynchronous version)
                new_config = await get_config_from_api_async(self.config)
                if new_config is None:
                    self.logger.bind(tag=TAG).error("Failed to get the new configuration")
                    return False
                self.logger.bind(tag=TAG).info(f"Successfully retrieved the new configuration")
                # Check whether the VAD and ASR types need to be updated
                update_vad = check_vad_update(self.config, new_config)
                update_asr = check_asr_update(self.config, new_config)
                self.logger.bind(tag=TAG).info(
                    f"Checking whether the VAD and ASR types need to be updated: {update_vad} {update_asr}"
                )
            ...
            self.logger.bind(tag=TAG).error(f"Failed to update the server configuration: {str(e)}")
            return False
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If the device is on the allowlist, skip token validation and allow it through
                return
            else:
