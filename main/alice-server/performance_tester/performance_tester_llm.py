...
description = "LLM performance test"


class LLMPerformanceTester:
    def __init__(self):
        self.config = load_config()
        # Use test content that is more aligned with agent scenarios, including a system prompt
        self.system_prompt = self._load_system_prompt()
        self.test_sentences = self.config.get("module_test", {}).get(
            "test_sentences",
            [
                "Hello, I feel a bit down today. Could you comfort me?",
                "Could you check the weather for tomorrow?",
                "I want to hear an interesting story. Can you tell me one?",
                "What time is it now? What day of the week is it today?",
                "I want to set an alarm for 8:00 tomorrow morning to remind me of a meeting.",
            ],
        )
        self.results = {}
    ...
    def _collect_response_sync(self, llm, messages, llm_name, sentence_start):
        """Collect response data synchronously"""
        chunks = []
        ...
        try:
            ...
            for chunk in response_generator:
                chunk_count += 1
                # Check whether the request should be interrupted after every few chunks
                if chunk_count % 10 == 0:
                    # Exit early by checking whether the current thread is marked for interruption
                    import threading
                    ...
                    if (
                        ...
                        # If it is not the main thread, check whether it should stop
                        pass
            ...
            # For 502 errors or network errors, raise the exception so the caller can handle it
            if (
                "502" in error_msg
    ...
    async def _check_ollama_service(self, base_url: str, model_name: str) -> bool:
        """Check the Ollama service status asynchronously"""
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(f"{base_url}/api/version") as response:
                    if response.status != 200:
                        print(f"Ollama service is not running or cannot be reached: {base_url}")
                        return False
                async with session.get(f"{base_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        if not any(model["name"] == model_name for model in models):
                            print(
                                f"Ollama model {model_name} was not found. Please run `ollama pull {model_name}` first."
                            )
                            return False
                    else:
    ...
    async def _test_single_sentence(
        ...
        try:
            ...
            # Build a message containing the system prompt
            messages = [
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    ...
                    try:
                        ...
                        future.cancel()
                        # Wait briefly to ensure the thread-pool task can respond to cancellation
                        try:
                ...
                print(f"{llm_name} exception while handling the request: {timeout_error}")
                return None
    ...
    async def _test_llm(self, llm_name: str, config: Dict) -> Dict:
        """Run an asynchronous performance test for a single LLM"""
        try:
            ...
            if llm_name == "Ollama":
                ...
                if not model_name:
                    print("Ollama is not configured with model_name")
                    return {
                        "name": llm_name,
                        "type": "llm",
                        "errors": 1,
            ...
            # Run all sentence tests concurrently and handle possible exceptions
            ...
            if not valid_results:
                print(f"{llm_name} has no valid data; the issue may be network-related or caused by configuration errors")
                return {
                    "name": llm_name,
                    "type": "llm",
                    "errors": 1,
            ...
            # Check the number of valid results; if too few, treat the test as failed
            if len(valid_results) < len(test_sentences) * 0.3:  # At least 30% success rate
                print(
                    f"{llm_name} has too few successful sentence tests ({len(valid_results)}/{len(test_sentences)}); the network may be unstable or the API may have issues"
                )
                return {
                    "name": llm_name,
                    "type": "llm",
            ...
            # Filter outliers (more than 3 standard deviations)
    ...
    def _print_results(self):
        ...
        if not self.results:
            print("No test results are available")
            return
        ...
        # Convert the sorted valid results into table data
        ...
        # Add the error results to the end of the table data
        table_data.extend(error_results)

        print(tabulate(table_data, headers=headers, tablefmt="grid"))
        print("\nTest notes:")
        print("- Test content: agent dialogue scenarios with a complete system prompt")
        print("- Timeout control: the maximum wait time for each request is 10 seconds")
        print("- Error handling: automatically skip models with 502 errors and network anomalies")
        print("- Success rate: number of successfully responded sentences / total test sentences")
        print("\nTesting completed!")
    ...
    async def run(self):
            ...
            for llm_name, config in self.config.get("LLM", {}).items():
                ...
                if llm_name == "CozeLLM":
                    if any(x in config.get("bot_id", "") for x in ["your"]) or any(
                        ...
                        print(f"LLM {llm_name} is not configured with bot_id/user_id, skipping")
                        continue
                elif "api_key" in config and any(
                    x in config["api_key"] for x in ["your", "placeholder", "sk-xxx"]
                ):
                    print(f"LLM {llm_name} is not configured with api_key, skipping")
                    continue
        ...
        # Run all test tasks concurrently, with an independent timeout for each task
        ...
        for result in all_results:
            if isinstance(result, dict):
                if result.get("errors") == 0:
                    ...
                    # Even errors are recorded so failed states can be displayed
                    if result.get("name") != "Unknown":
                        self.results[result["name"]] = result
            elif isinstance(result, Exception):
                print(f"Error while processing test results: {str(result)}")
