...
class BaseASRTester:
    ...
    def _calculate_result(self, service_name, latencies, test_count):
        """Calculate test results (Fix: Correctly handle None values, exclude failed tests)"""
        # Exclude None values (failed tests) and invalid latencies, only count valid latencies
...
class DoubaoStreamASRTester(BaseASRTester):
    ...
    async def test(self, test_count=5):
        if not self.test_audio_files:
            return {"name": "Doubao Streaming ASR", "latency": 0, "status": "Failed: Test audio not found"}
        if not self.asr_config:
            return {"name": "Doubao Streaming ASR", "latency": 0, "status": "Failed: Not configured"}
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # Send audio data (using last frame flag to signal end of audio to server)
                    ...
                    print(f"[Doubao ASR] Test #{i+1} First word latency: {latency:.3f}s")
                    await ws.close()

            except Exception as e:
                print(f"[Doubao ASR] Test #{i+1} failed: {str(e)}")
...
class QwenASRFlashTester(BaseASRTester):
    ...
    async def _test_single(self, audio_file_info):
        ...
        try:
            ...
            # Optimization: Move temp file preparation before timing to reduce disk IO impact on performance testing
            ...
            # Unified timing start point: Start timing right before API call (after file preparation is complete)
            start_time = time.time()
            ...
            raise Exception("Streaming ended without receiving any response")
    ...
    async def test(self, test_count=5):
        if not self.test_audio_files:
            return {"name": "Tongyi Qianwen ASR", "latency": 0, "status": "Failed: Test audio not found"}
        if not self.asr_config and not os.getenv("DASHSCOPE_API_KEY"):
            return {"name": "Tongyi Qianwen ASR", "latency": 0, "status": "Failed: api_key is not configured"}

        latencies = []
        for i in range(test_count):
            try:
                # print(f"\n[Tongyi ASR] Starting test #{i+1}...")
                latency = await self._test_single(self.test_audio_files[0])
                latencies.append(latency)
                print(f"[Tongyi ASR] Test #{i+1} First word latency: {latency:.3f}s")
            except Exception as e:
                # print(f"[Tongyi ASR] Test #{i+1} failed: {str(e)}")
                latencies.append(None)
...
class XunfeiStreamASRTester(BaseASRTester):
    ...
    async def test(self, test_count: int = 5):
        if not self.test_audio_files:
            return {"name": "iFlytek Streaming ASR", "latency": 0, "status": "Failed: Test audio not found"}
        if not self.asr_config:
            return {"name": "iFlytek Streaming ASR", "latency": 0, "status": "Failed: Not configured"}

        required = ["app_id", "api_key", "api_secret"]
        for k in required:
            if k not in self.asr_config:
                return {"name": "iFlytek Streaming ASR", "latency": 0, "status": f"Failed: Missing configuration {k}"}
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    async for message in ws:
                        ...
                        if data.get("code") != 0:
                            raise Exception(f"iFlytek Error: {data.get('message')}")
                        ...
                        if ws_result:
                            ...
                            if text.strip() and first_token:
                                latency = time.time() - start_time
                                latencies.append(latency)
                                print(f"[iFlytek ASR] Test #{i+1} First word latency: {latency:.3f}s")
                                first_token = False
                                break

            except Exception as e:
                print(f"[iFlytek ASR] Test #{i+1} failed: {str(e)}")
                latencies.append(None)
...
class ASRPerformanceSuite:
    ...
    def _print_results(self, test_count):
        ...
        print(f"{'='*60}")
        print(f"Number of tests: Each ASR service tested {test_count} times")
        ...
        print("\nTest Notes:")
        print("- Timing start point: Before establishing connection (includes handshake, sending audio, and receiving initial result)")
        print("- Tongyi Qianwen optimization: Temp file preparation done before timing to reduce disk IO impact")
        print("- Error handling: Failed tests excluded from average latency calculation, counting only successful runs")
        print("- Sorting rule: Successful tests sorted by latency in ascending order, failed ones listed at the end")

    async def run(self, test_count=5):
        print(f"Starting Streaming ASR First Word Response Time Test...")
        print(f"Tests per ASR service: {test_count} times\n")
