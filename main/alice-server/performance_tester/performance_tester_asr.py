...
class ASRPerformanceTester:
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            if asr_config:
                                ...
                                print(f"[DEBUG] Loaded ASR config successfully from {file_path}")
    ...
    async def _test_single_audio(self, stt_name: str, stt, audio_data: bytes) -> Optional[float]:
        ...
        try:
            ...
            if abs(duration) < 0.001:  # Less than 1ms is treated as abnormal
    ...
    async def _test_stt_with_timeout(self, stt_name: str, config: Dict) -> Dict:
        ...
        try:
            ...
            if any(
                ...
                print(f"  STT {stt_name} valid access_token/api_key not configured, skipped")
                return {
                    "name": stt_name,
                    "type": "stt",
                    "errors": 1,
                ...
                try:
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        ...
                        if duration is not None and duration > 0.001:  
                            total_time += duration
                            valid_tests += 1
                            print(f" {stt_name} [{i}/{test_count}] Time elapsed: {duration:.2f}s")
                        else:
                            print(f" {stt_name} [{i}/{test_count}] Test failed (incl. 0.000s anomaly)")
                            
                except asyncio.TimeoutError:
                    print(f" {stt_name} [{i}/{test_count}] Timeout (10s), skipping")
                    continue
                except Exception as e:
                    error_msg = str(e).lower()
                    if "502" in error_msg or "bad gateway" in error_msg:
    ...
    def _print_results(self):
        ...
        # Sort in ascending order by response time (fastest to slowest)
        ...
        print("\nTest Notes:")
        print("- Timeout control: Max wait time for a single audio file is 10 seconds")
        print("- Error handling: Automatically skips models with 502 errors, timeouts, and network exceptions")
        print("- Success rate: Number of successfully recognized audio files / total test audio files")
        print("- Sorting rule: Sorted by average duration from fast to slow, failed models listed last")
        print("\nTest completed!")

    async def run(self):
        """Run full asynchronous test""" 
        print("Starting filtering available ASR modules...")
        ...
        for stt_name, config in self.config["ASR"].items():
            ...
            if any(
                ...
                print(f"ASR {stt_name} missing valid access_token/api_key, skipped")
                continue
