...
description = "Streaming TTS First-Word Latency Performance Test"
class StreamTTSPerformanceTester:
    ...
    async def test_aliyun_tts(self, text=None, test_count=5):
        """Test Aliyun Streaming TTS first-word latency (run multiple tests and take the average)"""
        ...
        for i in range(test_count):
            try:
                ...
                # Unified timing starting point: start timing before establishing connection
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # 2. Wait for task-started event (Critical! Must wait for this before sending text)
                    task_started = False
                    ...
                    # 3. Send continue-task (Send text! This is the correct action)
                    ...
                    # 4. Send finish-task (Finish task)
                    finish_task_message = {
                        "header": {
                            "action": "finish-task",
                            "task_id": session_id,
    ...
    async def test_doubao_tts(self, text=None, test_count=5):
        """Test ByteDance Volcano Engine Streaming TTS first-word latency (run multiple tests and take average)"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(ws_url, additional_headers=ws_header, max_size=1000000000) as ws:
                    ...
                    latency = time.time() - start_time
                    latencies.append(latency)
                    print(f"[Volcano Engine TTS] Run {i+1} First-word latency: {latency:.3f}s")

            except Exception as e:
                print(f"[Volcano Engine TTS] Run {i+1} test failed: {str(e)}")
                latencies.append(None)
    ...
    async def test_paddlespeech_tts(self, text=None, test_count=5):
        """Test PaddleSpeech Streaming TTS first-word latency (run multiple tests and take average)"""
        text = text or self.test_texts[0]
        latencies = []
    ...
    async def test_indexstream_tts(self, text=None, test_count=5):
        """Test IndexStream Streaming TTS first-word latency (run multiple tests and take average)"""
        text = text or self.test_texts[0]
        latencies = []
    ...
    async def test_linkerai_tts(self, text=None, test_count=5):
        """Test Linkerai Streaming TTS first-word latency (run multiple tests and take average)"""
        text = text or self.test_texts[0]
        latencies = []
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    params = {
                        ...
                        "target_sr": 16000,
                        "audio_format": "pcm",
                        "instruct_text": "Please generate a piece of natural and fluent speech",
                    }
                    headers = {
    ...
    async def test_xunfei_tts(self, text=None, test_count=5):
        """Test iFLYTEK Streaming TTS first-word latency (run multiple tests and take average)"""
        text = text or self.test_texts[0]
        latencies = []
        
        for i in range(test_count):
            try:
                # Correct configuration node name to match XunFeiTTS in config file
                    ...
                    while not first_audio_received:
                        ...
                        if audio_payload:
                            ...
                            if status == 1 and audio_data:
                                ...
                                latency = time.time() - start_time
                                latencies.append(latency)
                                print(f"[iFLYTEK TTS] Run {i+1} First-word latency: {latency:.3f}s")
                                first_audio_received = True
                                break
            except Exception as e:
                print(f"[iFLYTEK TTS] Run {i+1} test failed: {str(e)}")
                latencies.append(None)
    ...
    def _calculate_result(self, service_name, latencies, test_count):
        """Calculate test results (properly handle None values, exclude failed tests)"""
        # Exclude failed tests (None values and <=0 latency), count only valid latencies
    ...
    def _print_results(self, test_text, test_count):
        ...
        print(f"Test count: {test_count} tests per TTS service")

        # Sort results: Successes first, sorted by latency ascending
        ...
        print("\nTest Notes: Measure time from connection setup to receiving the first audio chunk (includes handshake, auth, text sending), averaged over multiple tests")
        print("- Start point: Before establishing WebSocket/HTTP connection (includes connection setup, handshake, text sending process)")
        print("- Timeout control: Maximum wait time per request is 10 seconds")
        print("- Error handling: Failed tests are excluded from average; only successful tests are calculated")
        print("- Sorting rule: Sorted by average duration from fastest to slowest")
    ...
    async def run(self, test_text=None, test_count=5):
        ...
        Args:
            test_text: Text content to test; if None, default text will be used
            test_count: Number of tests per TTS service
        """
        test_text = test_text or self.test_texts[0]
        print(f"Starting streaming TTS first-word latency test...")
        print(f"Test text: {test_text}")
        print(f"Test count per TTS service: {test_count} times")
...
async def main():
    ...
    parser.add_argument("--count", type=int, default=5, help="Number of tests per TTS service")
