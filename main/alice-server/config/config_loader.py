...
async def get_config_from_api_async(config):
    """Get configuration from Java API (async version)"""
    ...
    # If the server has no prompt_template, read from local config
...
async def get_private_config_from_api(config, device_id, client_id):
    ...
    private_config = agent_result if isinstance(agent_result, dict) else {}
    if correct_words:
...
def ensure_directories(config):
    ...
    # Create all directories uniformly (keep original data directory creation)
...
def merge_configs(default_config, custom_config):
    """
    Recursively merge configs, custom_config has higher priority
