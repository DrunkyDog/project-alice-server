...
play_music_function_desc = {
    ...
    "function": {
        "name": "play_music",
        "description": "Called when the user requests to play music or a song.",
        "parameters": {
            "type": "object",
            "properties": {
                "song_name": {
                    "type": "string",
                    "description": "Song name. If the user does not specify a specific song name, use 'random'. If specified clearly, return the music name. Example: ```User: Play Two Tigers\nParameter: Two Tigers``` ```User: Play music\nParameter: random```",
                }
            },
            "required": ["song_name"],
        },
...
async def play_music(conn: "ConnectionHandler", song_name: str):
    try:
        ...
        conn.logger.bind(tag=TAG).error(f"Error handling the music intent: {e}")
        return ActionResponse(
...
def _extract_song_name(text):
    """Extract the song name from user input"""
...
async def handle_music_command(conn: "ConnectionHandler", text):
    ...
    if os.path.exists(MUSIC_CACHE["music_dir"]):
        ...
        if potential_song:
            ...
            if best_match:
                conn.logger.bind(tag=TAG).info(f"Found the closest matching song: {best_match}")
                await play_local_music(conn, specific_file=best_match)
                return True
    # Check whether this is a general music-play command
    await play_local_music(conn)
    return True


def _get_random_play_prompt(song_name):
    """Generate a random playback introduction"""
    ...
    prompts = [
        ...
        f"Let us listen together to \"{clean_name}\"",
        f"Please enjoy the next song, \"{clean_name}\"",
        f"Here is a special performance of \"{clean_name}\" for you",
    ]
    # Use random.choice directly without setting a seed
