...
// Automatically executed when the Service Worker is injected with the manifest
...
// Add install event handler to display installation message in the console
...
self.addEventListener('fetch', event => {
  ...
  if (isCDNEnabled) {
    ...
    // Output cache hit information for CDN resources
    if ([...CDN_CSS, ...CDN_JS].includes(url.href)) {
      // Do not interfere with the normal fetch flow, only add logs
    ...
    new workbox.strategies.CacheFirst({
      ...
      plugins: [
        new workbox.expiration.ExpirationPlugin({
          maxAgeSeconds: 365 * 24 * 60 * 60, // Increased to 1 year cache
...
workbox.routing.setCatchHandler(async ({ event }) => {
  ...
  switch (event.request.destination) {
    case 'document':
      // If it is a web page request, return the offline page
