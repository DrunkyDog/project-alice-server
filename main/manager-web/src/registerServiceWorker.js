    ...
    window.addEventListener('load', () => {
      ...
      navigator.serviceWorker.getRegistrations().then(registrations => {
        if (registrations.length > 0) {
          console.info('[Xiaozhi Service] Existing Service Worker registration found, checking for updates');
        }
        ...
        navigator.serviceWorker
              ...
              installingWorker.onstatechange = () => {
                if (installingWorker.state === 'installed') {
                  if (navigator.serviceWorker.controller) {
                    // Content has been cached and updated, notify the user to refresh
                    console.log('[Xiaozhi Service] New content available, please refresh the page');
                    // Update notification can be displayed here
                    ...
                    // Everything is normal, Service Worker successfully installed
                    console.log('[Xiaozhi Service] Content has been cached for offline use');
                    
                    // Cache can be initialized here
                    setTimeout(() => {
                      // Warm up CDN cache
                      const cdnUrls = [
          ...
          .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to register a ServiceWorker')) {
              console.warn('[Xiaozhi Service] Network error occurred while registering Service Worker, CDN resources may not be cacheable');
              if (process.env.NODE_ENV === 'production') {
                console.info(
                  'Possible reasons: 1. The server has not configured the correct MIME type 2. Server SSL certificate issue 3. The server did not return the service-worker.js file'
                );
              }
            }
          });
